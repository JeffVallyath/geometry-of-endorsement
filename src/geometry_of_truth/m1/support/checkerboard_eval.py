"""
Exact checkerboard evaluator + bootstrap CIs on the leakage effects.

WHY THIS EXISTS
The two paired metrics separately kill situation-only and consideration-only
explanations. They do NOT kill an ADDITIVE model:

    f(s, c) = a(s) + b(c)

b(c) carries it within situations, a(s) carries it within considerations, and
it never computes how THIS consideration applies to THIS situation. Both
aggregate metrics can look fine.

The reciprocal checkerboard closes that hole:

              c1        c2
      s1   Supports  Opposes
      s2   Opposes   Supports

Solving it requires ranking c1 > c2 in s1 AND c2 > c1 in s2. For an additive
model the gap is b(c1) - b(c2) in both situations — same sign — so it can
satisfy at most one. Two exact invariants follow:

    pairwise accuracy over the 2 comparisons : additive == 0.500 exactly
    fraction of boards solved BOTH ways      : additive == 0.000 exactly

The second is the sharper test. It is the only metric here that an additive
main-effects model cannot score above zero on.

Run:
    python checkerboard_eval.py --out checkerboards.json
"""
...
def main() -> None:
    ...
    print("=" * 78)
    # Mine on the EXACT consideration string, not the cluster.
    # Third instance of the same hazard: if the mining key is coarser than the
    # model's input, c1-in-s1 and c1-in-s2 can be DIFFERENT strings inside one
    # cluster, consideration_only assigns them different scores, and the
    # additive invariant silently breaks (it read 0.517 / 0.040 instead of
    # 0.500 / 0.000). The structural key must always match the input the
    ...
    print("\nread: any model whose 'both ways' CI excludes 0 is doing something")
    print("      an additive main-effects model provably cannot do.")
