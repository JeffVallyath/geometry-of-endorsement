"""Measure text shortcuts under strict splits and controlled overlap restoration.

The primary outcome is within-situation paired accuracy. Each intervention
changes one exposure rule while preserving the test draw, model, and metric.
"""
...
def l2_normalize(text: str) -> str:
    """Apply L1 normalization and strip the Value, Right, or Duty prefix.

    Prefixes encode consideration type. Token order remains unchanged because
    character n-gram similarity depends on local sequence.
    """
...
def l3_cluster(
    ...
    """Leader clustering over L2 forms without transitive chaining.

    Each form must match the cluster leader directly, and the cluster size cap
    limits over-merging. Frequent forms become leaders first. Character n-grams
    capture orthographic variants but cannot identify every semantic synonym.
    """
    counts = counts or {}
    ...
    # One bounded merge pass recovers direct leader matches while preserving the size cap.
...
def make_split(df: pd.DataFrame, condition: str, rng: np.random.Generator):
    ...
    if condition == "strict":
        ...
        # side are excluded and counted separately.
