"""RSO3 model-side adapter: the validated SRS2 Backbone (frozen BF16 backbone, FP32 readout, validated token boundaries, question-blind
compile -> immutable hashed key/value artifact, clone-served asks, next-block propagation witness) with a program workspace hook:

  workspace='fp32'   one FP32 working residual of the whole prefix is created from the block output inside the hook; every declared
                     command of the program is applied there in order (setters replace their addressed-clause matrix; affine maps add
                     cap-then-strength deltas; the affine clause summary z is the current FP32 clause mean); the workspace is cast to the
                     backbone dtype ONLY when written back into the block output at the union of edited positions.
  workspace='legacy' the exact historical V2 path: per command, BF16 states are read, the delta is computed in FP32 and the result is cast
                     to BF16 before the next command reads it (bit-identical anchor replay of the V2 rows).

`compile` has no question argument and serves batch-1 prefixes only (no query-aware arm exists in V3).
"""
...
import srs2_adapter as V2A
from srs2_adapter import validate_cache, parameter_checksum, tensor_bytes, BoundaryError  # noqa: F401
...
class Backbone(V2A.Backbone):
    ...
    def full_forward(self, prefix_ids, suffix_batch, label_ids_batch, prefix_maps=None, prefix_index=None, prefix_site=None, late=None, grad=True, replay=None, prefix_rows=None, prefix_batch=1, workspace='fp32'):
        """Prefix forward with the edit program (FP32 workspace by default) + batched suffix forward on a fresh per-step cache (training path)."""
        ...
        art = self.compile(prefix_ids, maps=maps, index=prefix_index, site=prefix_site, grad=grad and maps is not None and replay is None, capture={late['site']} if late else (), workspace=workspace)
        cache = art['cache']
        if late is not None: raise ValueError('no query-time editor in RSO3')
        ...
        self.plan = dict(edit=None, capture=set()); self._reset()
        ctx = contextlib.nullcontext() if grad else t.no_grad()
        try:
            with ctx:
                out = self.model.model(input_ids=ids, attention_mask=mask, past_key_values=cache, use_cache=True, return_dict=True)
            ...
            return dict(label_logps=sel, full_logps=lp, prefix_realized=art['realized'], maps=maps, prefix_len=P)
        finally:
            self.plan = None; self._reset()
