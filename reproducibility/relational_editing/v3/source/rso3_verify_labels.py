...
def main():
    ...
    for split in ('FIT', 'CAL', 'FINAL', 'WORKFLOW'):
        for s in Q.scenes(split):
            progs = Q.PROGRAMS if split in ('FINAL', 'WORKFLOW') else ('single', 'noop')
            for program in progs:
                ev = edited_values(s, Q.programs(s)[program]); src = [list(r) for r in s.values]
                for draw in (0, 1):
    ...
    if a.qdir:
        ...
        for att in qd['attempts']:
            for r in Q.read_rows(Path(a.qdir)/f"qualification_{att['procedure']}.jsonl"):
                ...
                world = [list(x) for x in s.values] if r['world'] == 'source' else edited_values(s, Q.programs(s)['single'])
                g = truth(world, spec)
